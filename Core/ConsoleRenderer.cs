...
public static class ConsoleRenderer
{
    ...
    private static ConsoleColor GetConsoleColor(Color color)
    {
        ...
        {
            return color.R >= 150 ? ConsoleColor.Red : ConsoleColor.DarkRed;
        }
        if (color.B - color.R > 100 && color.B - color.G > 100)
        {
            return color.B >= 150 ? ConsoleColor.Blue : ConsoleColor.DarkBlue;
        }
        if (color.G - color.R > 100 && color.G - color.B > 100)
        {
            return color.G >= 150 ? ConsoleColor.Green : ConsoleColor.DarkGreen;
        }
